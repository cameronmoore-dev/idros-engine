...
namespace idrs
{
    ...
    void SpriteUtils::setTextureCoords(Sprite &sprite, f32 x, f32 y, f32 w, f32 h)
    {
        if (w > 1.0f || h > 1.0f)
        {
            x /= sprite.material.texture->getWidth();
            w /= sprite.material.texture->getWidth();
            y /= sprite.material.texture->getHeight();
            h /= sprite.material.texture->getHeight();
        }
    ...
    void SpriteUtils::flipY(Entity entity)
    {
        ...
        sprite.vertices[2].uv.y = sprite.textureRect.h - sprite.vertices[2].uv.y;
        sprite.vertices[3].uv.y = sprite.textureRect.h - sprite.vertices[3].uv.y;
